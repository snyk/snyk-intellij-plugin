...
// Apply "copy to clipboard" functionality (via vendor/clipboard.min.js)
var clipboard = new Clipboard('[data-snyk-clipboard]');

// Change the trigger text to "Copied" when it's copied.
clipboard.on('success', function (e) {
  e.trigger.innerHTML = 'Copied';
});

// Some browsers don't support clipboard, so tell the user
// to copy the text in usual way.
clipboard.on('error', function (e) {
  var actionMsg = /Mac/i.test(navigator.userAgent) ?
    '⌘+C to copy' : 'Ctrl+C to copy';

  e.trigger.innerHTML = actionMsg;
});

if (/iPhone|iPad/i.test(navigator.userAgent)) {
  document.querySelectorAll('[data-snyk-clipboard]').forEach(function (el) {
...
/* global $ */
/**
 *  Each dialog as a minimum requires
 *  - A trigger element
 *
 *  Optionally you can declare an anchor element this defines where the modal
 *  will visibly appear within the viewport.
 */
function Dialog($el) {
  var _self = this;

  // move the element to the bottom of the document
  _self.$el = $el.detach();
  _self.$el.appendTo('body');

  // get the trigger element attribute
  _self.triggerElementAttribute = _self.$el.data('snykDialogTrigger')
    ? _self.$el.data('snykDialogTrigger')
    : '[href*="' + _self.$el.attr('id') + '"]';

  _self.$el.on('click', '.dialog__dismiss', function (evt) {
    evt.preventDefault();
    evt.stopPropagation();
    _self.hide();
  });

  // remove any previously set click events for the target elements.
  // we do this because after an ignore, this function is run again and the events
  // would just multiply.
  $('body').off('click', _self.triggerElementAttribute);
  // bind an event to the body which listens for a delegated click event on the anchor element
  // instead of binding directly to the element
  $('body').on('click', _self.triggerElementAttribute, function (evt) {
    evt.preventDefault();
    evt.stopPropagation();

    // get the trigger element
    _self.$trigger = $(_self.triggerElementAttribute);

    // set the anchor element to the trigger unless a different anchor is specified
    _self.$anchor = _self.$trigger;

    if (_self.$el.data('snyk-anchor')) {
      _self.$anchor = $(_self.$el.data('snyk-anchor'));
    }
    ...
    if (_self.isVisible()) {
      ...
      _self.positionElement(_self.$el.data('snykDialogPositioning'));

      // Open target dialog
      _self.$el.removeClass('dialog--hidden');
    }

  });

  $(window).on('click', function (evt) {
    // Hide dialog when clicking outside target
    if (_self.$el.has(evt.target).length === 0) {
      _self.hide();
    }
  });
...
Dialog.prototype = {
  hide: function () {
    return this.$el.addClass('dialog--hidden');
  },

  isVisible: function () {
    return this.$el.hasClass('dialog--hidden') === false;
  },

  positionElement: function (positioning) {
    var _self = this;
    var anchorPosition = _self.$anchor.offset();
    var dialogOffset = parseInt(_self.$el.data('snykDialogOffset'), 10) || 0;

    var props = {
      top: anchorPosition.top - (_self.$el.outerHeight() + dialogOffset),
      left: (anchorPosition.left + _self.$anchor.outerWidth()) - _self.$el.outerWidth()
    };

    if (positioning === 'tr') {
      props = {
        left: false,
        top: anchorPosition.top + _self.$anchor.outerHeight() + dialogOffset,
        right: $(document).outerWidth() - (anchorPosition.left + _self.$anchor.outerWidth())
      };
    }

    _self.$el.css($.extend({position: 'absolute'}, props));
  }
};


$(document).on('snyk:dialogSetup', function () {
  $('[data-synk-dialog]').each(function () {
    new Dialog($(this));
  });
});

$(document).trigger('snyk:dialogSetup');

var $linkLoadingEls = $('[data-snyk-link-loading]').each(function() {
  var $el = $(this);
  $el.data('el', $el.find($el.data('snyk-link-loading')));
});

$linkLoadingEls.on('click', function() {
  var $el = $(this);

  $linkLoadingEls.each(function() {
    $(this).data('el').addClass('loading-spinner--hidden');
  });

  if ($el.data('el')) {
    $el.data('el').removeClass('loading-spinner--hidden');
  }
});

/* global Slideout */

var $sidepanel = $('[data-snyk-sidepanel]');


// Only work for wide screens
if ($sidepanel.length && window.innerWidth > 780) {

  var loadingHtml = '<div class="loading-spinner loading-spinner--lg">' +
    '<div class="loading-spinner__image"><svg width="40" height="40" ' +
    'viewBox="0 0 24 24" role="img" aria-labelledby="svg-spinner">' +
    '<title id="svg-spinner">Loading data</title><path d="M20 12h4c0-6.' +
    '6-5.4-12-12-12v4c4.4 0 8 3.6 8 8z"/><path d="M12 0c6.6 0 12 5.4 12 ' +
    '12s-5.4 12-12 12S0 18.6 0 12 5.4 0 12 0zm0 20c4.4 0 8-3.6 8-8s-3.6-8-' +
    '8-8-8 3.6-8 8 3.6 8 8 8z" opacity=".2"/></svg></div><div ' +
    'class="loading-spinner__text">Loading</div></div>';

  var sidepanel = new Slideout({
    panel: document.querySelector('[data-snyk-main]'),
    menu: document.querySelector('[data-snyk-sidepanel]'),
    side: 'right',
    padding: 700,
  });

  $(document).on('click', '[data-snyk-sidepanel-trigger]', function (e) {
    var url = $(this).data('snyk-sidepanel-trigger');
    var href = $(this).attr('href');
    $(sidepanel.menu).html(loadingHtml).load(url, function (response, status) {
      $(document).trigger('snyk:sidepanelLoaded', url);
      if (status === 'error') {
        var message = '<p>Couldn&apos;t load content</p>';
        if (href) {
          message += '<p><a href="' + href + '">Try again</a></p>';
        }

        $(sidepanel.menu).html(message);
      }
    });
  ...
  sidepanel.on('beforeopen', function () {
    $(document).trigger('snyk:sidepanelBeforeOpen');
    this.panel.classList.add('sidepanel__main--open');
  }).on('open', function () {
    this.panel.addEventListener('click', close);
  }).on('beforeclose', function () {
    $(document).trigger('snyk:sidepanelBeforeClose');
    this.panel.classList.remove('sidepanel__main--open');
    this.panel.removeEventListener('click', close);
  });
}

// JS Accessibility layer: adds interactive
// aria-hidden statuses and keyboard functionality.
(function () {
  var ESCAPE_KEY = 27;

  var tooltip = '[data-snyk-tooltip]';
  var tooltipDescription = '[role=tooltip]';

  $(document)
    .on('click', tooltip, function (e) {
      e.preventDefault();
    })
    .on('mouseenter focus click touch touchstart touchmove touchend', tooltip,
      function () {
        $(this).find(tooltipDescription).attr('aria-hidden', 'false').show();
      })
    .on('mouseleave blur keydown touchstart', tooltip,
      function (e) {
        if (e.type === 'keydown') {
          if (e.which === ESCAPE_KEY) {
            $(this).find(tooltipDescription).attr('aria-hidden', 'true').hide();
          }
        } else {
          $(this).find(tooltipDescription).attr('aria-hidden', 'true').hide();
        }
...
// Test render
var $btns = $('[data-snyk-loading-button]');

$btns.parents('form').attr('data-snyk-async-form', true);

$(document).on('submit', '[data-snyk-async-form]', function() {
  var $form = $(this);
  var $btn = $form.find('[data-snyk-loading-button-trigger]');
  var $el = $form.find('[data-snyk-loading-button]').first();
  var $status = $el.find('[data-snyk-loading-button-status]');

  $el.data('store', $btn.text());
  ...
  $el.addClass('s__loading');

  $status
    .removeClass('hidden')
    .attr('role', 'alert')
    .text($el.data('snyk-waiting'))
    .focus();

  $form.one('snyk:complete', function(evt, str) {
    $btn.removeAttr('style').text($el.data('store'));
    ...
    if (str) {
      ...
      $status.attr('role', false).hide();
    }

    $el.removeClass('s__loading');
  });
});

$(document).on('change', '[data-snyk-dependents] input', function() {
  var $trigger = $(this);
  var $parent = $trigger.parents('[data-snyk-dependents]').first();
  var $inputs = $parent.find('input');

  if ( $inputs.index($trigger) === 0 ) {
    $inputs.not($trigger).each(function() {
