...
class SnykProjectSettingsConfigurable(val project: Project) : SearchableConfigurable {
  ...
  override fun isModified(): Boolean {
    ...
    return isCoreParamsModified() ||
      ...
      snykSettingsDialog.manageBinariesAutomatically() !=
        settingsStateService.manageBinariesAutomatically ||
  ...
  private fun isPreferredOrgModified(): Boolean {
    ...
    return (isProjectSettingsAvailable(project) && dialogPreferredOrg != storedPreferredOrg)
  }
...
/**
 * Utility function to apply folder config changes based on dialog settings. This encapsulates the
 * logic for updating preferredOrg and orgSetByUser based on the auto-select org checkbox state.
 */
...
/**
 * Common post-apply logic shared between old dialog and new HTML settings panel. Refreshes feature
 * flags, updates LS configuration, and fires settings events.
 */
...
/**
 * Handles release channel change by prompting user to download new CLI. Shared between old dialog
 * and new HTML settings panel. Only prompts if manageBinariesAutomatically is enabled, since manual
 * mode users manage their own CLI.
 */
...
/**
 * Handles delta findings (issues to display) change by clearing caches and updating tree. Shared
 * between old dialog and new HTML settings panel.
 */
