  ...
  fun downloadFile(cliFile: File, cliVersion: String, indicator: ProgressIndicator): File {
    ...
    val downloadFile =
      try {
        ...
        val message =
          "Cannot create file in the configured CLI path directory ${cliFile.parent}. " +
    ...
    try {
      ...
      ProjectUtil.getOpenProjects().forEach { project ->
        ...
        try {
          ...
          logger.debug(
            "CliDownloader: exception during LS shutdown for ${project.name}, continuing: ${e.message}"
          )
        }
