...
/**
 * Wrap work with Snyk CLI. See
 * [Kotlin's generic type's magic](https://kotlinlang.org/docs/generics.html).
 */
abstract class CliAdapter<CliIssues, R : CliResult<CliIssues>>(val project: Project) {
  ...
  private val projectPath: String =
    project.basePath
      ?: throw IllegalStateException(
        "Scan should not be performed on Default project (with `null` project base dir)"
      )
  ...
  fun execute(commands: List<String>): R =
    try {
      ...
      cmds.removeAt(0)
      val rawResultStr =
        LanguageServerWrapper.getInstance(project).executeCLIScan(cmds, projectPath)
  ...
  protected abstract fun getProductResult(
    cliIssues: List<CliIssues>?,
    snykErrors: List<SnykError> = emptyList(),
  ): R

  /**
   * `Gson().fromJson` could put `null` value into not-null field. That case should be
   * handled(tested/checked) here and (specific) Exception thrown if needed.
   */
   ...
   * !!! Public for tests only !!!
   *
   * If result string not contains 'error' string and contain 'vulnerabilities' it says that
   * everything is correct. If result string not contains '{' it means CLI return an error. And if
   * result string contains 'error' and not contain 'vulnerabilities' it means CLI return error in
   * JSON format. if result == [ConsoleCommandRunner.PROCESS_CANCELLED_BY_USER] - CLI scan process
   * was terminated by user. if result is Empty - CLI fail to run (some notification already done in
   * CLI execution code).
   */
  ...
  private fun convertSingleEntryToCliResult(rawStr: String): R =
    when {
      isSuccessCliJsonString(rawStr) ->
        getResultOrError {
          // we should catch all exceptions here including JsonParseException, JsonSyntaxException,
          // etc.
  ...
  private fun convertArrayToCliResult(rawStr: String): R = getResultOrError {
    // see
  ...
  protected open fun isSuccessCliJsonString(jsonStr: String): Boolean =
    jsonStr.contains("\"vulnerabilities\":")
