  ...
  fun scan() {
    runInBackground("Snyk: triggering scan", project, true) {
      ...
      if (!isCliInstalled()) {
        ...
        getSnykCliDownloaderService()
          .errorHandler
          .showErrorWithRetryAndContactAction(
  ...
  fun waitUntilCliDownloadedIfNeeded() {
    ...
    val connection =
      ApplicationManager.getApplication()
        .messageBus
        .connect(SnykPluginDisposable.getInstance(project))
    try {
      ...
      try {
        ...
        if (!cliInstalled) {
          ...
          if (force || !integrityOk) {
            logger.debug(
              "Triggering CLI download: forceUpdate=$force, integrityCheckFailed=${!integrityOk}"
            )
            cliDownloader.downloadLatestRelease(indicator, project)
          } else {
