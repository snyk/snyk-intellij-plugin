...
object SnykBalloonNotificationHelper {
  ...
  private fun showNotification(
    ...
    // workaround for
    // https://youtrack.jetbrains.com/issue/IDEA-220408/notifications-with-project=null-is-not-shown-anymore
    if (project != null) {
  ...
  private fun createBalloon(message: String, icon: Icon, color: Color): Balloon =
    JBPopupFactory.getInstance()
      .createHtmlTextBalloonBuilder(message, icon, color, null)
  ...
  fun showInfoBalloonForComponent(
    message: String,
    component: Component,
  ...
  fun showWarnBalloonAtEventPlace(message: String, e: AnActionEvent, showAbove: Boolean = false) {
    val component =
      e.inputEvent?.component ?: e.getData(PlatformDataKeys.CONTEXT_COMPONENT) ?: return
    // todo: case if no Component exist (action invoked from Search?)
    showWarnBalloonForComponent(message, component, showAbove)
  }

  fun showWarnBalloonForComponent(
    message: String,
    component: Component,
    showAbove: Boolean = false,
  ) {
    val balloon =
      createBalloon(message, AllIcons.General.BalloonWarning, MessageType.WARNING.popupBackground)
