...
class SnykSettingsDialog(
  ...
  private val tokenTextField =
    JBPasswordField().apply { preferredSize = Dimension(600, preferredSize.height) }
  private val receiveTokenButton = JButton("Connect IDE to Snyk")
  private val authenticationType =
    ComboBox(
        AuthenticationType.entries.sortedBy { it.dialogIndex }.map { it.dialogName }.toTypedArray()
      )
      .apply { this.isEditable = false }

  private val customEndpointTextField =
    JTextField().apply { preferredSize = Dimension(600, preferredSize.height) }
  private val organizationTextField: JTextField =
    JTextField().apply {
      toolTipText =
        "Global organization setting. This field is used when auto-select organization is disabled in the Project settings."
      preferredSize = Dimension(600, preferredSize.height)
    }
  private val ignoreUnknownCACheckBox: JCheckBox =
    JCheckBox().apply {
      toolTipText = "Enabling this causes SSL certificate validation to be disabled"
    }

  private val scanOnSaveCheckbox =
    JCheckBox().apply {
      toolTipText = "If enabled, automatically scan on save, start-up and configuration change"
    }
  private val additionalParametersTextField: JTextField =
    ExpandableTextField().apply {
      toolTipText = "--all-projects is already defaulted, -d causes problems"
    }
  ...
  private val autoDetectOrgCheckbox: JCheckBox =
    JCheckBox().apply {
      ...
      addActionListener { updatePreferredOrgTextField() }
    }
  ...
  // We have to do background task run through Alarm on Alarm.ThreadToUse.POOLED_THREAD due to next
  // (Idea?) bug:
  // Creation of Task.Backgroundable under another Task.Backgroundable does not work for Settings
  // dialog,
  ...
  private fun initializeUiComponents() {
    ...
    generalSettingsPanel.add(
      ...
      baseGridConstraints(row = 3, column = 0, indent = 0, anchor = UIGridConstraints.ANCHOR_WEST),
    )
    ...
    generalSettingsPanel.add(
      receiveTokenButton,
      baseGridConstraintsAnchorWest(row = 5, column = 1, indent = 0),
    )

    val tokenLabel = JLabel("Token:")
    generalSettingsPanel.add(tokenLabel, baseGridConstraintsAnchorWest(row = 6, indent = 0))
    ...
    generalSettingsPanel.add(organizationLabel, baseGridConstraintsAnchorWest(row = 7, indent = 0))
    ...
    issueViewPanel.add(issueViewLabelPanel, baseGridConstraintsAnchorWest(row = 0, indent = 0))
    ...
    if (isProjectSettingsAvailable(project)) {
      ...
      projectSettingsPanel.add(
        autoDetectOrgLabel,
        baseGridConstraintsAnchorWest(row = 1, indent = 0),
      )
      ...
      projectSettingsPanel.add(
        preferredOrgLabel,
        baseGridConstraintsAnchorWest(row = 2, indent = 0),
      )
    ...
    userExperiencePanel.add(
      scanOnSaveCheckbox,
      baseGridConstraints(row = 0, anchor = UIGridConstraints.ANCHOR_NORTHWEST, indent = 0),
    )

    /** Spacer ------------------ */
    val generalSettingsSpacer = Spacer()
    rootPanel.add(generalSettingsSpacer, panelGridConstraints(row = 5))
  }

  private fun createNetNewPanel(productAndSeveritiesPanel: JPanel) {
    val netNewIssuesPanel =
      JPanel(UIGridLayoutManager(2, 2, JBUI.insets(Insets(5, 0, 20, 0)), -1, -1))
    ...
    val netNewIssuesText =
      JLabel("Specifies whether to see only net new issues or all issues.").apply {
        font = FontUtil.minusOne(this.font)
      }
  ...
  private fun createExecutableSettingsPanel() {
    ...
    executableSettingsPanel.add(introLabel, gb.nextLine())

    cliBaseDownloadUrlTextField.toolTipText =
      "The default URL is https://downloads.snyk.io. " +
    ...
    val descriptionLabelReleaseChannel =
      HyperlinkLabel("Find out about our release channels").apply {
        setHyperlinkTarget("https://docs.snyk.io/snyk-cli/releases-and-channels-for-the-snyk-cli")
      }
