...
class SeveritiesEnablementPanel {
  ...
  val panel =
    panel {
        row {
          checkBox(Severity.CRITICAL.toPresentableString())
            .applyToComponent { name = text }
            .actionListener { _, it ->
              val isSelected = it.isSelected
              if (canBeChanged(it, isSelected)) {
                // we need to change the settings in here in order for the validation to work
                // pre-apply
                currentCriticalSeverityEnabled = isSelected
              }
            }
            // bindSelected is needed to trigger apply() on the settings dialog that this panel is
            // rendered in
            // that way we trigger the re-rendering of the Tree Nodes
            .bindSelected(settings::criticalSeverityEnabled)
        }
        row {
          checkBox(Severity.HIGH.toPresentableString())
            .applyToComponent { name = text }
            .actionListener { _, it ->
              val isSelected = it.isSelected
              if (canBeChanged(it, isSelected)) {
        ...
        row {
          checkBox(Severity.MEDIUM.toPresentableString())
            .applyToComponent { name = text }
            .actionListener { _, it ->
              val isSelected = it.isSelected
              if (canBeChanged(it, isSelected)) {
        ...
        row {
          checkBox(Severity.LOW.toPresentableString())
            .applyToComponent { name = text }
            .actionListener { _, it ->
              val isSelected = it.isSelected
              if (canBeChanged(it, isSelected)) {
