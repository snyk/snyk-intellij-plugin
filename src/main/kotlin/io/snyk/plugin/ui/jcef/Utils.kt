...
object JCEFUtils {
  ...
  /**
   * Generates a cryptographically secure nonce for CSP. Uses URL-safe Base64 encoding without
   * padding to avoid special characters.
   */
  ...
  /**
   * Creates a new JCEF browser with standard configuration. The caller is responsible for disposing
   * the browser.
   *
   * @param offScreenRendering Use off-screen rendering (default: true). Set to true for lightweight
   *   rendering without native window.
   */
  ...
  /**
   * Creates a JCEF browser with load handlers but WITHOUT loading HTML. Use this when you want to
   * defer HTML loading to avoid EDT blocking. Call loadHTML() separately, preferably via
   * invokeLater.
   *
  ...
  fun getJBCefBrowserComponentIfSupported(
    ...
    if (!JBCefApp.isSupported()) {
      ...
      SnykBalloonNotificationHelper.showWarn(
        "JCEF is not supported on this platform, we cannot display issue details",
        null,
      )
      return null
    }
  ...
  /**
   * Creates a JCEF browser and loads HTML content. HTML loading is deferred via invokeLater to
   * avoid blocking the EDT during browser initialization.
   *
