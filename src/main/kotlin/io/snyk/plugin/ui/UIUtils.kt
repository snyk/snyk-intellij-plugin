...
/**
 * Be careful! On macOS it's height could be set to 0 in some cases: would blame `fill =
 * FILL_HORIZONTAL`, but sometimes even with pure `panelGridConstraints` it's not shown (height=0)
...
fun getReadOnlyClickableHtmlJEditorPane(
  ...
  getReadOnlyClickableHtmlJEditorPaneFixedSize(htmlText, font, noBorder).apply {
...
fun getReadOnlyClickableHtmlJEditorPaneFixedSize(
  ...
  return JEditorPane("text/html", "<html>$htmlText</html>").apply {
    isEditable = false
    ...
    val bodyRule =
      UIUtil.displayPropertiesToCSS(font, fontColor) +
    ...
    (document as HTMLDocument)
      .styleSheet
      .addRule("h1, h2, h3, h4 { font-size: 1.1em; margin-bottom: 0em; }")
...
private fun makeOpaque(component: JComponent) {
  ...
  component.foreground =
    JBColor(
      Color(component.foreground.red, component.foreground.green, component.foreground.blue, alpha),
      Color(component.foreground.red, component.foreground.green, component.foreground.blue, alpha),
    )
}
...
fun txtToHtml(s: String): String {
  ...
  val newLineConverted = escapedHtml.replace("\n", "<br>").replace("\t", "&nbsp; &nbsp; &nbsp;")
