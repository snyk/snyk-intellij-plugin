...
class ProgressManager(val project: Project) : Disposable {
  ...
  private fun report(report: WorkDoneProgressReport, progressIndicator: ProgressIndicator) {
  ...
  fun notifyProgress(params: ProgressParams) {
    ...
    if (progress == null) {
      // The server is not spec-compliant and reports progress using server-initiated progress but
      // didn't
      // call window/workDoneProgress/create beforehand. In that case, we check the 'kind' field of
      // the
