...
class IgnoreService(project: Project) : CliAdapter<Unit, IgnoreService.IgnoreResults>(project) {
  ...
  fun ignore(issueId: String) {
    ...
    if (isFail(result)) {
      throw IgnoreException(
        "Unexpected error (${result.getFirstError()?.message}) when ignoring $issueId."
      )
    }
  }
  ...
  class IgnoreResults(errorMessage: String?) :
    CliResult<Unit>(null, errorMessage?.let { listOf(SnykError(it, "")) } ?: emptyList()) {
