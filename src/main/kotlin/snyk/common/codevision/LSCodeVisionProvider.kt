...
class LSCodeVisionProvider : CodeVisionProvider<Unit>, CodeVisionGroupSettingProvider {
  ...
  override fun isAvailableFor(project: Project): Boolean {
    ...
    return pluginSettings.ossScanEnable ||
      pluginSettings.snykCodeSecurityIssuesScanEnable ||
      pluginSettings.iacScanEnabled
  }
  ...
  override fun computeCodeVision(editor: Editor, uiData: Unit): CodeVisionState {
    ...
    val virtualFile =
      FileDocumentManager.getInstance().getFile(document) ?: return CodeVisionState.READY_EMPTY
    ...
    val hasResults =
      cachedResults?.currentOSSResultsLS?.get(snykFile)?.isNotEmpty() == true ||
    ...
    val codeLenses =
      try {
        lsWrapper.languageServer.textDocumentService.codeLens(params).get(timeout, TimeUnit.SECONDS)
  ...
  private class LSCommandExecutionHandler(private val codeLens: CodeLens) :
    ...
    override fun invoke(event: MouseEvent?, editor: Editor) {
      ...
      val task =
        object : Task.Backgroundable(project, "Executing ${codeLens.command.title}", false) {
