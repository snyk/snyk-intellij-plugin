...
object DocumentChanger {
  ...
  fun applyChange(change: Map.Entry<String, List<TextEdit>>?) {
    ...
    logger.debug(
      "applyChange: Attempting to find file by URI: $fileURI (length: ${fileURI.length})"
    )
    ...
    val virtualFile =
      fileURI.toVirtualFileOrNull()
        ?: return logger.warn("applyChange: File not found for URI: $fileURI")

    val document = virtualFile.getDocument() ?: return
    // Our LS is coded to give us the TextEdits in ascending order, but we must apply them in
    // descending order.
    // Imagine we had an edit that added 10 lines to the start of the file followed by an edit that
    // added a line to the 4th line of the file,
    // ascending order applying would incorrectly place the second edit withing the 10 lines added
    // at the start!
    // N.b. Line edits on a single line are returned as two edits, a deletion of the line and an
    // addition to the line after, so reversing works for this too.
    // N.b. The previous note is a lie for edits at the end of the file when no LF at EOF is
    // present, for those the addition is marked for the same line as the deletion.
    ...
    for (e in change.value.reversed()) {
      ...
      if (endLine > document.lineCount) {
        ...
        endCharacter = document.getLineEndOffset(endLine) - document.getLineStartOffset(endLine)
      }
