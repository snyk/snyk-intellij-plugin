...
import io.snyk.plugin.client.{CliClient, SnykUserInfo}
...
object SegmentApi {
  ...
  def apply(apiClient: CliClient): SegmentApi = new LiveSegmentApi(apiClient)
}
...
class LiveSegmentApi(apiClient: CliClient) extends SegmentApi {
  import SegmentApi._
