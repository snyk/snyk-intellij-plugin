...
class TreeNodeExpanderTest : LightPlatform4TestCase() {
  ...
  fun `expandProgressively with empty list should invoke callback`() {
    ...
    ApplicationManager.getApplication().invokeAndWait {
      cut.expandProgressively(rootNode, emptyList()) { callbackInvoked.set(true) }
    }
  ...
  fun `expandProgressively with null list should invoke callback`() {
    ...
    ApplicationManager.getApplication().invokeAndWait {
      cut.expandProgressively(rootNode, null) { callbackInvoked.set(true) }
    }
  ...
  fun `expandProgressively with small node count should invoke callback immediately`() {
    ...
    repeat(5) { i -> rootNode.add(DefaultMutableTreeNode("child$i")) }
    (tree.model as DefaultTreeModel).reload()
    ...
    ApplicationManager.getApplication().invokeAndWait {
      cut.expandProgressively(rootNode, userObjects) { callbackInvoked.set(true) }
    }
  ...
  fun `expandProgressively with medium node count should invoke callback after chunks`() {
    ...
    repeat(30) { i -> rootNode.add(DefaultMutableTreeNode("child$i")) }
    (tree.model as DefaultTreeModel).reload()
  ...
  fun `expandProgressively with large node count should invoke callback`() {
    ...
    repeat(100) { i -> rootNode.add(DefaultMutableTreeNode("child$i")) }
    (tree.model as DefaultTreeModel).reload()
  ...
  fun `expandInChunks should invoke callback after all chunks`() {
    // Given: a tree with children
    repeat(25) { i -> rootNode.add(DefaultMutableTreeNode("child$i")) }
    (tree.model as DefaultTreeModel).reload()
    ...
    ApplicationManager.getApplication().invokeAndWait {
      cut.expandInChunks(rootNode, userObjects) { callbackInvoked.set(true) }
    }
  ...
  fun `expandToDepth should invoke callback`() {
    ...
    ApplicationManager.getApplication().invokeAndWait {
      cut.expandToDepth(2) { callbackInvoked.set(true) }
    }
  ...
  fun `expander should stop processing when disposed`() {
    ...
    disposed.set(true)
    ApplicationManager.getApplication().invokeAndWait {
      disposableExpander.expandInChunks(rootNode, userObjects) { callbackInvoked.set(true) }
    }
